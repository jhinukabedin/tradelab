        ...
        <div class="col-12">
            <div class="row gy-4">
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.report.transaction', $user->id) }}" title="Balance" icon="las la-money-bill-wave-alt"
                        value="{{ showAmount($user->balance) }}" bg="indigo" type="2"  outline="true" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.deposit.list', $user->id) }}" title="Deposits" icon="las la-wallet"
                        value="{{ showAmount($totalDeposit) }}" bg="8" type="2"  outline="true" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.withdraw.data.all', $user->id) }}" title="Withdrawals" icon="la la-bank"
                        value="{{ showAmount($totalWithdrawals) }}" bg="6" type="2"  outline="true" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.report.transaction', $user->id) }}" title="Transactions" icon="las la-exchange-alt"
                        value="{{ $totalTransaction }}" bg="17" type="2"  outline="true" />
                </div>
            </div>
            <div class="row gy-4 mt-1">
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.trade.log.index') }}?user_id={{ $user->id }}" title="Total Trades"
                        icon="las la-gamepad" value="{{ $widget['total_trade'] }}" bg="primary" type="2" outline="true" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.trade.log.wining') }}?user_id={{ $user->id }}" title="Win Trades"
                        icon="las la-trophy" value="{{ $widget['total_wining_trade'] }}" bg="success" type="2" outline="true" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.trade.log.losing') }}?user_id={{ $user->id }}" title="Loss Trades"
                        icon="las la-slash" value="{{ $widget['total_losing_trade'] }}" bg="danger" type="2" outline="true" />
                </div>
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.trade.log.draw') }}?user_id={{ $user->id }}" title="Draw Trades"
                        icon="las la-draw-polygon" value="{{ $widget['total_draw_trade'] }}" bg="info" type="2" outline="true" />
                </div>
            </div>
                                    ...
                                    <select name="country" class="form-control select2">
                                        @foreach ($countries as $key => $country)
                                            <option data-mobile_code="{{ $country->dial_code }}" value="{{ $key }}"
                                                @selected($user->country_code == $key)>
