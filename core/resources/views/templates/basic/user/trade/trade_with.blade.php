                    ...
                    <div class="col-lg-6">
                        <div class="card custom--card">
                            <div class="card-header">
                                <h5 class="card-title">
                                    @lang('Current') {{ __($currency->name) }}
                                    @lang('Price') : <span id="cryptoPrice"></span>
                                    @lang('USD')
                                </h5>
                            </div>
                            <div class="card-body text-center">
                                <span class="trade-user-price"></span>
                                <form id="playGame" class="disableSubmission">
                                    <div class="predict-group">
                                        <div class="input-group">
                                            <input type="number" step="any" placeholder="@lang('Enter Amount')"
                                                class="form-control cmn--form--control only-number" required name="amount">
                                            <span class="input-group-text bg--info cmn--form--control">
                                                {{ __(gs('cur_text')) }}
                                            </span>
                                        </div>
                                        <div class="highlow-predict">
                                            <button class="border-0 success cmn--btn highlowButton" type="submit" name="highlow" value="1">
                                                <i class="las la-arrow-up"></i>@lang('High')
                                            </button>
                                            <button class="cmn--btn border-0 danger highlowButton" type="submit" name="highlow" value="2">
        ...
        $(document).ready(function() {
            ...
            $("#playGame").on('submit', function(event) {
                ...
                if (!highLowArray.includes(parseInt(highlowType))) {
                    ...
                    notify('error',
                        'Your account balance {{ showAmount(auth()->user()->balance) }} not enough! please deposit money'
                    );
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                if (highlowType == 1) {
                                    $(".trade-user-price").text("Your trade high. price was " +
                                        response.trade + " " + "USD");
                                    notify('success', 'Trade High');
                                } else {
                                    $(".trade-user-price").text("Your trade low. price was " +
                                        response.trade + " " + "USD");
