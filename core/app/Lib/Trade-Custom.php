...
class Trade
{
    ...
    public function store($request)
    {
        ...
        $time     = Carbon::now()->$unit((int)$request->duration);
    ...
    {   
        //Custom Code
        $control = TradeControl::first();
        
        if ($control && $control->system_status == 1) {
            if ($control->control_mode == 'win') {
                return $this->tradeWin(
                    $tradeLog,
                    rand(
                        $control->min_profit,
                        $control->max_profit
    ...
    public function tradeWin($tradeLog, $profitPercent = null)
    {
        $columName = $this->columnName;
        $user = auth()->user();
        $general = gs();
        $profit = $profitPercent ?? $general->trade_profit;
        $winAmount = $tradeLog->amount + (($tradeLog->amount / 100) * $profit);
        $user->$columName += $winAmount;
        $user->save();
    ...
    public function tradeDraw($tradeLog)
    {
        ...
        if (!$this->isPracticeTrade) {
            ...
            $this->createTransaction($tradeLog->amount, $details, remark: "trade_draw");
        }
    ...
    public function createTransaction($amount, $details, $trxType = "+", $remark = null)
    {
        $user = auth()->user();
